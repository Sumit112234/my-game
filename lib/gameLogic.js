...
export const TIMER_OPTIONS = [30, 60, 90, 120, 180]
...
/**
 * Check if myHead collides with any segment of otherSnake's body (not head).
 * Returns true if my head bit into their body.
 */
export function checkBodyBite(mySnake, otherSnake) {
  if (!mySnake || !otherSnake || mySnake.length === 0 || otherSnake.length < 2) return false
  const head = mySnake[0]
  // Check against all segments except the head of otherSnake
  return otherSnake.slice(1).some(seg => seg.x === head.x && seg.y === head.y)
}

/**
 * Check if two snakes are colliding head-to-head.
 */
export function checkHeadOnCollision(snakeA, snakeB) {
  if (!snakeA || !snakeB || snakeA.length === 0 || snakeB.length === 0) return false
  const hA = snakeA[0]
  const hB = snakeB[0]
...
/**
 * Shrink a snake by n segments (minimum 1 segment left).
 */
export function shrinkSnake(snake, n) {
  const newLength = Math.max(1, snake.length - n)
  return snake.slice(0, newLength)
}

/**
 * Grow snake by n extra segments (duplicate tail).
 */
export function growSnakeBy(snake, n) {
  const tail = snake[snake.length - 1]
  const extras = [...Array(n)].map(() => ({ ...tail }))
  return [...snake, ...extras]
}
...
export function spawnFood(snakes, existingFood) {
  ...
  Object.values(snakes).forEach(snake => {
    if (snake) snake.forEach(seg => occupied.add(`${seg.x},${seg.y}`))
  })
